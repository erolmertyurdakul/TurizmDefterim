const Map<String, dynamic> alternatifTurizmUnit1 = {
  "title": "Alternatif Turizme Giriş",
  "learningUnit": "1. Öğrenme Birimi",
  "podcastUrl": "https://anchor.fm/s/114a64400/podcast/play/122580106/https%3A%2F%2Fd3ctxlq1ktw2nl.cloudfront.net%2Fstaging%2F2026-6-8%2Fa3258fed-11a0-a8b2-1375-f8795eb473b5.mp3",
  "cards": [
    {
      "id": 1,
      "tag": "KAVRAM",
      "title": "ALTERNATİF TURİZM KAVRAMI VE ORTAYA ÇIKIŞI",
      "microSummary": "Doğal kaynakların kitle turizmiyle plansız kullanımına tepki olarak doğan, çevreye ve yerel kültüre duyarlı, sürdürülebilir turizm şeklidir.",
      "definitions": [
        {
          "name": "Alternatif Turizm",
          "desc": "Kitle turizmine (deniz-kum-güneş) alternatif olarak ortaya çıkan, turizmin sürdürülebilir şekilde 12 aya yayılmasını destekleyen, sosyal ve ekolojik uyuma, yerel girişimcilerin iş birliğine ve gelişmede yerli malzeme kullanılmasına öncelik veren turizm çeşididir. 'Yumuşak (soft)', 'yeşil', 'yavaş', 'sorumlu' turizm olarak da adlandırılır.",
          "examples": [
            "Örnekle Pekiştirelim: Turistlerin 5 yıldızlı devasa beton oteller yerine, yöre halkının işlettiği ahşap pansiyonlarda kalarak doğaya zarar vermeden tatil yapmaları alternatif turizme en güzel örnektir."
          ]
        },
        {
          "name": "Kitle Turizmi",
          "desc": "Deniz-kum-güneş (3S - Sea, Sand, Sun) üçlüsüne bağlı, standart ürün ve hizmetlerin kitlelere sunulduğu, çevreye daha çok baskı yapan turizm türüdür. 'Sert (hard)' turizm olarak da anılır.",
          "examples": [
            "Örnekle Pekiştirelim: Binlerce turistin aynı anda Antalya'daki her şey dahil devasa bir otele gidip, sadece otelde yiyip içerek plajı kullanması klasik bir kitle turizmidir."
          ]
        }
      ],
      "extraDetails": [
        {
          "title": "Neden Ortaya Çıktı?",
          "content": "• Çevre bilincinin artması ve sürdürülebilirlik anlayışının gelişmesi.\n• Kitle turizminin yarattığı geri dönülemez doğa tahribatları.\n• Turizmin 12 aya yayılma hedefi.\n• Tüketicilerin klasik, kalabalık tatillerden sıkılıp daha bireysel ve keşfetmeye yönelik deneyimler araması.\n• Turizm gelirlerinin kıyı şeridi dışındaki bölgelere de yayılması isteği."
        }
      ],
      "caseStudy": "Bir tatil yöresinde binlerce kişinin aynı anda sahili kullanıp çöp bırakması ve her şey dahil konseptte otelden dışarı çıkmaması (Kitle Turizmi) yerel ekonomiye az katkı sağlar. Ancak az sayıda doğa severin yöresel restoranlarda yemek yiyip dağ yürüyüşü yapması (Alternatif Turizm) hem ekosistemi korur hem de yöre halkına doğrudan gelir sağlar.",
      "tip": "Kitle turizmi = Standart Paket, Çok İnsan, Çevreye Baskı. Alternatif Turizm = Bireysel/Küçük Grup, Özgün Deneyim, Doğa Dostu."
    },
    {
      "id": 2,
      "tag": "KARŞILAŞTIRMA",
      "title": "KİTLE TURİZMİ VE ALTERNATİF TURİZMİN FARKLARI",
      "microSummary": "Kitle turizmi büyük sermayeye ve standart hizmetlere dayanırken, alternatif turizm yerele, özgünlüğe ve sürdürülebilirliğe odaklanır.",
      "definitions": [
        {
          "name": "Mekân ve Zaman",
          "desc": "Kitle turizmi sahillerde ve yaz aylarında yoğunlaşırken; alternatif turizm her bölgede geliştirilebilir ve tüm yıla yayılabilir.",
          "examples": [
            "Örnekle Pekiştirelim: Kıyı otellerinin kışın kapanması zamansal bağımlılığa; ancak Bolu'da kışın kayak, baharda göl yürüyüşü, yazın ise yayla turları yapılması alternatif turizmin zamansızlığına örnektir."
          ]
        },
        {
          "name": "Yerel Halk ile Etkileşim",
          "desc": "Kitle turizminde turist otelden çıkmadığı için yerel halkla etkileşim zayıftır. Alternatif turizmde ise turist yerel halkın içinde olduğu için etkileşim ve iletişim yoğundur.",
          "examples": [
            "Örnekle Pekiştirelim: Her şey dahil otelde kalan turist yemeklerini otelde yer (Kitle). Gastronomi turisti ise köydeki teyzenin yaptığı testi kebabını yer (Alternatif)."
          ]
        },
        {
          "name": "Maliyet ve Ekonomi",
          "desc": "Kitle turizmi grup bazlı olduğu için turist açısından daha ucuza mal olur ama gelirin %80'i uluslararası şirketlere gider. Alternatif turizm bireye yönelik olduğu için maliyetlidir ama turist daha çok harcama yapar ve gelirin %90'ı yerel ekonomiye kalır.",
          "examples": [
            "Örnekle Pekiştirelim: Dev gemilerle (kruvaziyer) limana inen turistlerin sadece hediyelik alması kitle turizmidir. Bir köye gidip orada rehber tutup köy evinde kalan alternatif turist ise parasını doğrudan halka bırakır."
          ]
        }
      ],
      "extraDetails": [
        {
          "title": "Özellikleri Nelerdir?",
          "content": "• Taşıma kapasitesini dikkate alır (Aşırı kalabalığa izin verilmez).\n• Küçük ölçekli ve yerel yatırımlara öncelik verilir.\n• Doğal, tarihî ve kültürel kaynakları korur.\n• Özgür ve bağımsız gezginler tercih eder.\n• Çevreye ve kültürel değerlere saygılıdir."
        }
      ],
      "tip": "Kitle turizminin odağı 'Tüketim' iken, alternatif turizmin odağı 'Deneyim ve Koruma'dır."
    },
    {
      "id": 3,
      "tag": "EKONOMİ",
      "title": "ALTERNATİF TURİZMİN ÜLKE EKONOMİSİNE KATKILARI",
      "microSummary": "Turizm bir görünmez ihracattır. Döviz girdisi sağlar, istihdamı artırır ve bölgeler arası gelişmişlik farkını kapatır.",
      "definitions": [
        {
          "name": "Ödemeler Dengesine Etkisi",
          "desc": "Turizmden elde edilen döviz gelirleri, dış borçların ödenmesinde ve ithalat-ihracat dengesindeki cari açığın kapatılmasında büyük rol oynar.",
          "examples": [
            "Örnekle Pekiştirelim: Türkiye'nin turizm sezonunda elde ettiği 40 milyar dolarlık gelir, ülkeye sıcak döviz girdisi sağlayarak dış ticaret açığının kapanmasına doğrudan yardım eder."
          ]
        },
        {
          "name": "İş Gücü Çarpanı Etkisi",
          "desc": "Turizm sektöründeki büyümenin, tarım, ulaşım, inşaat gibi bağlı diğer tam 41 alt sektörü de hareketlendirerek ülke genelinde işsizliği azaltmasıdır.",
          "examples": [
            "Örnekle Pekiştirelim: Otele gelen turist sadece otelciye para kazandırmaz. Otelin yemek aldığı çiftçi, yatak örtüsü aldığı tekstilci ve transferi yapan şoför de kazanır. Buna çarpan etkisi denir."
          ]
        },
        {
          "name": "Gelir Dağılımını Düzenleyici Etkisi",
          "desc": "Geri kalmış ancak doğal güzelliğe sahip kırsal bölgelerin turizm sayesinde gelişerek zenginleşmesi ve kentle arasındaki refah farkının azalmasıdır.",
          "examples": [
            "Örnekle Pekiştirelim: Rize'nin küçük bir köyünde yaşayan köylülerin, yayla turizmi sayesinde evlerini butik otele çevirip şehre göç etmek yerine kendi köylerinde refah içinde yaşamasıdır."
          ]
        },
        {
          "name": "İstihdam Artışı",
          "desc": "Turizm, otomasyonun (robotlaşmanın) zor olduğu 'emek yoğun' bir sektördür. Bu yüzden her zaman insan gücüne ihtiyaç duyar.",
          "examples": [
            "Örnekle Pekiştirelim: Bir fabrikada yüzlerce robot üretim yapabilirken, bir restoranda garsonluk veya otelde kat görevlisi hizmetinin mutlaka insan eliyle ve samimiyetle yapılmasıdır."
          ]
        }
      ],
      "caseStudy": "Türkiye, Turizm Stratejisi 2023 Eylem Planı ile turist başına düşen ortalama harcamayı artırmayı hedefler. Sadece turist sayısının artması yetmez, turistlerin çok harcama yapacağı Golf, Kongre ve Sağlık turizmi gibi alanlar desteklenmelidir."
    },
    {
      "id": 4,
      "tag": "KORİDORLAR",
      "title": "BÖLGESEL TURİZM GELİŞİM KORİDORLARI",
      "microSummary": "Türkiye'nin turizmini 12 aya ve tüm ülkeye yaymak için 2023 Eylem Planı'nda belirlenmiş 7 adet tematik turizm gelişim koridorudur.",
      "definitions": [
        {
          "name": "Zeytin Koridoru",
          "desc": "Güney Marmara'da sağlık ve gastronomi turizmi odaklı gelişim planıdır (Zeytin ve zeytinyağı potansiyeli).",
          "examples": [
            "Örnekle Pekiştirelim: Balıkesir Ayvalık ve Bursa Mudanya hattında zeytin hasadı festivalleri düzenlenmesi ve zeytinyağı tadım turları yapılması bu koridorun faaliyetidir."
          ]
        },
        {
          "name": "Kış Koridoru",
          "desc": "Erzincan, Erzurum, Ağrı, Kars, Ardahan illerini kapsayan kış turizmi merkezidir.",
          "examples": [
            "Örnekle Pekiştirelim: Doğu Ekspresi ile Kars'a giden turistlerin Sarıkamış'ta kayak yapıp, Çıldır Gölü'nde buzda balık tutması kış koridorunun en güzel örneğidir."
          ]
        },
        {
          "name": "İnanç Turizmi Koridoru",
          "desc": "Tarsus, Hatay, Gaziantep, Şanlıurfa ve Mardin yörelerini kapsayan, dini ve tarihi yapıların restore edilerek değerlendirildiği hattır.",
          "examples": [
            "Örnekle Pekiştirelim: Şanlıurfa'daki Balıklıgöl veya Hatay'daki St. Pierre Kilisesi inanç koridoru için turist çeken en önemli cazibe merkezleridir."
          ]
        },
        {
          "name": "İpek Yolu Turizm Koridoru",
          "desc": "Ayaş-Sapanca, Adapazarı, Bolu ve Ankara illerini kapsayan tarihî ticaret yolunun canlandırılmasıdır.",
          "examples": [
            "Örnekle Pekiştirelim: Bolu Göynük ve Ankara Beypazarı'ndaki tarihi Osmanlı konaklarının restore edilip butik otellere çevrilerek kültür turlarının buraya yönlendirilmesidir."
          ]
        },
        {
          "name": "Batı Karadeniz Kıyı Koridoru",
          "desc": "Şile-Sinop arasında uzanan, Ankara ve İstanbul gibi metropollere hizmet edecek kültür, kıyı ve doğa turizmi şerididir.",
          "examples": [
            "Örnekle Pekiştirelim: Amasra veya Safranbolu'ya günübirlik ya da hafta sonu için giden İstanbullu turistlerin oradaki balıkçıları ve tarihi konakları ziyaret etmesidir."
          ]
        },
        {
          "name": "Yayla Koridoru",
          "desc": "Samsun'dan Hopa'ya kadar uzanan Karadeniz doğa ve yayla turizmi şerididir.",
          "examples": [
            "Örnekle Pekiştirelim: Trabzon Uzungöl, Rize Ayder ve Artvin Kafkasör yaylalarının birbirine entegre yollarla bağlanıp büyük doğa turlarına ev sahipliği yapmasıdır."
          ]
        },
        {
          "name": "Trakya Kültür Koridoru",
          "desc": "Edirne, Kırklareli ve Tekirdağ illerini kapsayan, özellikle Edirne evlerinin butik otellere çevrildiği kültür turizmi projesidir.",
          "examples": [
            "Örnekle Pekiştirelim: Edirne'deki Selimiye Camii'ni görmek, Kırkpınar güreşlerini izlemek ve meşhur Edirne tava ciğerini yemek için Trakya'ya düzenlenen turlardır."
          ]
        }
      ],
      "tip": "Koridor = Birbirine bağlanan komşu illerin aynı 'tema' etrafında turizme açılması."
    }
  ]
};

const Map<String, dynamic> alternatifTurizmUnit2 = {
  "title": "Alternatif Turizm Çeşitleri",
  "learningUnit": "2. Öğrenme Birimi",
  "podcastUrl": "https://anchor.fm/s/114a64400/podcast/play/122580133/https%3A%2F%2Fd3ctxlq1ktw2nl.cloudfront.net%2Fstaging%2F2026-6-8%2F8cfac79b-fe0d-667b-7987-e6cb112512b0.mp3",
  "cards": [
    {
      "id": 1,
      "tag": "SAĞLIK",
      "title": "SAĞLIK VE TERMAL TURİZMİ",
      "microSummary": "Kişinin bedenen ve ruhen iyileşmek veya güzelleşmek için yaptığı seyahatlerdir. Gelişmiş ülkelerdeki pahalı tedavilerden kaçış bu turizmi hızla büyütmüştür.",
      "definitions": [
        {
          "name": "Medikal Turizm (Tıp Turizmi)",
          "desc": "Ameliyatlar, estetik cerrahi, diş, göz tedavileri ve organ nakli gibi tıbbi müdahale gerektiren hizmetlerdir.",
          "examples": [
            "Örnekle Pekiştirelim: İngiltere'deki diş tedavi fiyatlarını çok pahalı bulan bir turistin, tedavisini Antalya'da yaptırıp bir hafta da tatil yapması medikal turizmdir."
          ]
        },
        {
          "name": "Termal Turizm",
          "desc": "Termomineral su (kaplıca), çamur banyosu ve iklim kürü kullanılarak yapılan doğal tedavilerdir. Türkiye Avrupa'da 1., Dünyada 7. sıradadır (Örn: Afyon, Denizli, Yalova).",
          "examples": [
            "Örnekle Pekiştirelim: Romatizma hastası bir yaşlının, şifa bulmak ve doktor kontrolünde çamur banyosu yapmak için Afyonkarahisar'daki 5 yıldızlı bir termal otele gitmesidir."
          ]
        },
        {
          "name": "Balneoterapi",
          "desc": "Termomineral sular, şifalı çamurlar veya gazların belirli kürler halinde (genellikle 14-21 gün, günde 1 veya 2 banyo şeklinde) vücuda banyo veya içme şeklinde uygulanmasıyla yapılan yasal kaplıca tedavi yöntemidir.",
          "examples": [
            "Örnekle Pekiştirelim: Doktor kontrolündeki bir misafirin, 15 gün boyunca her sabah termal havuzda 20 dakika kalıp mineral emilimini sağlaması ve su içme kürü uygulamasıdır."
          ]
        },
        {
          "name": "Klimatoterapi",
          "desc": "Temiz hava, güneş ışığı ve deniz havası gibi iklim faktörlerinin, doktor gözetiminde solunum, dolaşım ve deri hastalıklarının tedavisi amacıyla hedeflenen bölgede kullanılmasıdır.",
          "examples": [
            "Örnekle Pekiştirelim: Astım hastası bir misafirin, havası nemsiz ve oksijeni bol olan Kaz Dağları'nda doktor tavsiyesiyle 3 hafta kalarak solunum kürü yapmasıdır."
          ]
        },
        {
          "name": "Peloidoterapi (Çamur Tedavisi)",
          "desc": "Organik ve inorganik maddeler içeren şifalı doğal çamurların (peloid) lokal olarak ısıtılarak (genellikle 40-45 derece sıcaklıkta) vücuda bölgesel sarılması veya banyo şeklinde uygulanmasıdır.",
          "examples": [
            "Örnekle Pekiştirelim: Eklem ağrısı olan misafirin, kaplıca tesisindeki peloidoterapi ünitesinde dizlerine sıcak çamur sürdürüp 30 dakika sargıda bekleterek şifa aramasıdır."
          ]
        },
        {
          "name": "SPA & Wellness Turizmi",
          "desc": "SPA (Sudan gelen sağlık) ve Wellness (iyilik hali, zindelik). Cilt bakımı, masaj, yosun banyosu gibi bedeni ve ruhu dinlendiren hizmetlerdir.",
          "examples": [
            "Örnekle Pekiştirelim: Yoğun iş temposunda çalışan bir yöneticinin, hafta sonu zihnini dinlendirmek için Sapanca'daki bir otele gidip masaj yaptırması ve sauna kullanmasıdır."
          ]
        },
        {
          "name": "Yaşlı ve Engelli Turizmi (Geriatri Turizmi)",
          "desc": "İleri yaş bireylerin ve engelli bireylerin bakımı, rehabilitasyonu için yapılan seyahatlerdir.",
          "examples": [
            "Örnekle Pekiştirelim: İskandinav ülkelerindeki emekli ve engelli derneklerinin, üyelerini fizyoterapi ve tekerlekli sandalye dostu otellere sahip Antalya'ya tatile göndermesidir."
          ]
        }
      ],
      "tip": "Sağlık turisti, klasik bir turiste oranla çok daha fazla harcama yapar ve ülke ekonomisine büyük katkı sağlar."
    },
    {
      "id": 2,
      "tag": "DOĞA VE SPOR",
      "title": "KIŞ, HAVA VE DOĞA SPORLARI TURİZMİ",
      "microSummary": "Doğayla iç içe olmayı ve adrenali seven turistlerin tercih ettiği, ekipman ve beceri gerektiren turizm çeşitleridir.",
      "definitions": [
        {
          "name": "Kış Sporları Turizmi",
          "desc": "Karlı dağlarda yapılan kayak, snowboard, kar motoru (snowmobile) gibi aktiviteleri kapsar. Maliyeti yüksek bir turizm çeşididir. En popüler merkezler: Palandöken (Erzurum), Uludağ (Bursa), Erciyes (Kayseri), Kartalkaya (Bolu), Sarıkamış (Kars).",
          "examples": [
            "Örnekle Pekiştirelim: Sömestr tatilinde kayak takımlarını alıp ailesiyle birlikte Bursa Uludağ'daki telesiyejli ve karlı pistleri olan bir otele tatile giden bir gencin yaptığı spordur."
          ]
        },
        {
          "name": "Kayak Pisti Zorluk Dereceleri",
          "desc": "Kayak pistlerinin eğimlerine ve zorluklarına göre dünya standartlarında renklendirilmesidir: Yeşil Pist (%10'dan az eğim, yeni başlayanlar), Mavi Pist (%10-25 eğim, orta düzey), Kırmızı Pist (%25-40 eğim, iyi düzey) ve Siyah Pist (%40'tan fazla dik eğim, uzman kayakçılar).",
          "examples": [
            "Örnekle Pekiştirelim: Hayatında ilk kez kayak yapacak olan Ahmet'in, eğitmen eşliğinde en güvenli ve düz olan Yeşil Pist'te alıştırmalar yapması; deneyimli abisinin ise dik ve engebeli olan Kırmızı veya Siyah Pist'i tercih etmesidir."
          ]
        },
        {
          "name": "Hava Sporları Turizmi",
          "desc": "Yamaç paraşütü (Babadağ/Fethiye), sıcak hava balonu (Kapadokya), yelken kanat, planör ve skydiving gibi adrenali yüksek aktivitelerdir.",
          "examples": [
            "Örnekle Pekiştirelim: Muğla Fethiye Babadağ'dan profesyonel bir pilot eşliğinde yamaç paraşütüyle atlayıp Ölüdeniz'in eşsiz manzarasını gökyüzünden izleyen bir turistin deneyimidir."
          ]
        },
        {
          "name": "Dağcılık ve Kaya Tırmanışı",
          "desc": "Fiziksel güç gerektiren, yüksek zirvelere veya dik kayalıklara tırmanmayı içeren spordur. Ağrı Dağı, Kaçkarlar ve Aladağlar (Niğde) en önemli merkezlerdir.",
          "examples": [
            "Örnekle Pekiştirelim: Özel ekipmanlar ve dağcı ipleriyle Niğde Aladağlar'ın dik yamaçlarına tırmanış düzenleyen bir dağcılık kulübünün spor seyahatidir."
          ]
        },
        {
          "name": "Su Altı Dalış Turizmi",
          "desc": "Bitki, hayvan zenginliği ve batıkları görmek için yapılır. Türkiye'de Antalya (Kaş, Kemer) ve Çanakkale (Saros) çok popülerdir.",
          "examples": [
            "Örnekle Pekiştirelim: Antalya Kaş'ta oksijen tüpü takıp Akdeniz'in mavi sularına dalarak su altındaki antik batıkları ve deniz kaplumbağalarını (Caretta Caretta) fotoğraflamaktır."
          ]
        },
        {
          "name": "Akarsu Turizmi",
          "desc": "Rafting, kano ve nehir kayağı. Çoruh Nehri (Artvin), Köprüçay (Antalya), Dalaman Çayı ve Fırtına Deresi (Rize) rafting merkezleridir.",
          "examples": [
            "Örnekle Pekiştirelim: Rize Fırtına Deresi'nin azgın ve hızlı sularında, 6 kişilik şişme bota binip kürek çekerek dalgalarla mücadele eden bir arkadaş grubunun macerasıdır."
          ]
        }
      ],
      "caseStudy": "Kapadokya bölgesinde yılda 500 bin turist sabahın erken saatlerinde sadece Sıcak Hava Balonuna binmek için ödeme yapmaktadır. Bu da hava sporları turizminin ne kadar büyük bir potansiyel olduğunu gösterir."
    },
    {
      "id": 3,
      "tag": "KÜLTÜR",
      "title": "KÜLTÜR, İNANÇ VE İPEK YOLU TURİZMİ",
      "microSummary": "Geçmişin izlerini, gelenekleri, ibadet merkezlerini ve tarihi ticaret yollarını keşfetmek amacıyla yapılan seyahatlerdir.",
      "definitions": [
        {
          "name": "Kültür Turizmi",
          "desc": "Tarihi eserler, arkeolojik sit alanları, müzeler, yerel mimari ve el sanatlarını görmek için yapılan gezilerdir. Göbeklitepe, Zeugma, Efes Antik Kenti ve Çatalhöyük en güzel örnekleridir.",
          "examples": [
            "Örnekle Pekiştirelim: Şanlıurfa'ya gidip Göbeklitepe tapınak kalıntılarını gezen ve ardından Zeugma Mozaik Müzesi'ndeki 'Çingene Kızı' mozaiğini inceleyen bir turistin gezisidir."
          ]
        },
        {
          "name": "İnanç Turizmi",
          "desc": "Dini açıdan kutsal sayılan mekânları ziyaret etmektir. Müslümanlar için Hac ve Umre, Hristiyanlar için Meryem Ana Evi (İzmir) veya St. Nicholas Kilisesi (Antalya) ziyaretleri bu kapsamdadır.",
          "examples": [
            "Örnekle Pekiştirelim: Dünyanın dört bir yanından gelen Hristiyanların, İzmir Selçuk'taki kutsal Meryem Ana Evi'ni ziyaret edip dilek duvarına kağıt asması inanç turizmidir."
          ]
        },
        {
          "name": "İpek Yolu Turizmi",
          "desc": "Tarihî İpek Yolu güzergâhında bulunan han ve kervansarayların restore edilerek turizme kazandırılmasıdır (Örn: Nevşehir Sarı Han, Aksaray Sultan Hanı).",
          "examples": [
            "Örnekle Pekiştirelim: Aksaray'daki devasa Sultanhanı Kervansarayı'nı gezen turistlerin, Selçuklu mimarisini ve kervanların konaklama tarihini öğrenmesidir."
          ]
        },
        {
          "name": "Diaspora (Kopuntu) Turizmi",
          "desc": "Savaş, göç veya ekonomik nedenlerle vatanından kopmuş insanların, yıllar sonra kökenlerini bulmak ve akrabalarını ziyaret etmek için ana vatanlarına yaptıkları turizmdir.",
          "examples": [
            "Örnekle Pekiştirelim: 1960'larda Almanya'ya işçi olarak giden gurbetçilerin çocuklarının, her yaz memleketleri Sivas'a gelerek akrabalarını ziyaret edip hasret gidermesidir."
          ]
        }
      ],
      "tip": "Kültür turistleri eğitim ve gelir düzeyi yüksek, seyahat ettikleri bölgede daha uzun kalan ve bol harcama yapan turist profilidir."
    },
    {
      "id": 4,
      "tag": "KIRSAL",
      "title": "KIRSAL, EKOTURİZM VE YAYLA TURİZMİ",
      "microSummary": "Köy hayatını yaşamak, doğanın dengesini korumak ve şehir hayatından kaçıp sükunet bulmak amacıyla kırsal alanlara yapılan seyahatlerdir.",
      "definitions": [
        {
          "name": "Kırsal Turizm",
          "desc": "Şehir hayatının stresinden kaçarak kırsal bölgelerdeki köy ve çiftlik evlerinde kalıp, tarımsal yaşama dâhil olmaktır.",
          "examples": [
            "Örnekle Pekiştirelim: İstanbul'un kalabalığından kaçan bir ailenin, Kırklareli'nin bir köyündeki kerpiç evde kalıp, sabah yerel halkla birlikte köy ekmeği pişirmesidir."
          ]
        },
        {
          "name": "Çiftlik (Agro) Turizmi",
          "desc": "Turistlerin çiftliklerde konaklayıp salça yapımı, meyve toplama, inek sağma gibi tarımsal faaliyetlere fiilen katılıp yardım etmeleridir (TaTuTa Projesi).",
          "examples": [
            "Örnekle Pekiştirelim: Muğla'daki ekolojik bir çiftliğe giden turistin, sabah erkenden kalkıp domates toplaması, inek sağması ve kendi topladığı ürünlerle kahvaltı hazırlamasıdır."
          ]
        },
        {
          "name": "Ekoturizm",
          "desc": "Doğal alanlara, çevreye ve yerel halkın refahına zarar vermeden sorumluluk bilinciyle yapılan doğa seyahatleridir.",
          "examples": [
            "Örnekle Pekiştirelim: Doğa koruma altındaki bir sulak alana giden turistlerin, kuş gözlem kulelerinden dürbünle göçmen kuşları rahatsız etmeden izlemesi ve sadece patikalarda yürümesidir."
          ]
        },
        {
          "name": "Yayla Turizmi",
          "desc": "Özellikle Karadeniz bölgesinde serin hava, temiz su ve doğa ile iç içe yaşamak için yaylalara yapılan seyahatlerdir (Örn: Ayder, Pokut, Kafkasör).",
          "examples": [
            "Örnekle Pekiştirelim: Rize Çamlıhemşin'deki Pokut Yaylası'na bulut denizi manzarasını izlemek ve geleneksel ahşap yayla evlerinde konaklamak için çıkan turistlerin gezisidir."
          ]
        }
      ],
      "caseStudy": "İzmir'in Şirince köyü, yerel el sanatları ve meyve bağcılığıyla ekoturizm ve kırsal turizmi başarıyla uygulayıp tüm halkını kalkındırmıştır."
    },
    {
      "id": 5,
      "tag": "GÜNCEL",
      "title": "YENİ NESİL ALTERNATİF TURİZM AKIMLARI",
      "microSummary": "Turistlerin değişen psikolojisi, teknoloji ve çevre faktörleriyle ortaya çıkan sıra dışı turizm çeşitleri.",
      "definitions": [
        {
          "name": "Dark (Hüzün) Turizm",
          "desc": "Savaş, ölüm, doğal felaket ve acı olayların yaşandığı yerleri ziyaret etmektir. Örn: Çernobil, Auschwitz Toplama Kampı, Gelibolu, Kayaköy.",
          "examples": [
            "Örnekle Pekiştirelim: Çanakkale Gelibolu Tarihi Alanı'nı ziyaret edip, şehitlikleri ve savaş cephelerini görerek o dönemki acıları ve kahramanlıkları yerinde hissetmektir."
          ]
        },
        {
          "name": "Kıyamet Turizmi",
          "desc": "Küresel ısınma nedeniyle yakın gelecekte yok olacak veya sular altında kalacak yerleri 'son bir şans' ile görmek için yapılan seyahatlerdir. Örn: Venedik, Büyük Set Resifi, Maldivler.",
          "examples": [
            "Örnekle Pekiştirelim: Küresel ısınma sebebiyle 50 yıl içinde sular altında kalacağı tahmin edilen Maldiv adalarına gidip, bu güzelliği kaybolmadan önce son kez görmektir."
          ]
        },
        {
          "name": "Glamping Turizmi",
          "desc": "Glamorous (Büyüleyici) ve Camping (Kamp) kelimelerinin birleşimidir. Doğanın içinde, ancak 5 yıldızlı otel lüksü ve konforu sunan büyük ve şık çadırlarda tatil yapmaktır.",
          "examples": [
            "Örnekle Pekiştirelim: Kaz Dağları'nın ortasında, içinde kliması, özel banyosu ve lüks yatağı olan dev şeffaf bir çadırda (dome) yıldızları izleyerek kamp yapmaktır."
          ]
        },
        {
          "name": "Yavaş Turizm",
          "desc": "Hayatın hızına ve tüketim çılgınlığına tepki olarak ortaya çıkan; yavaş yemeği, yerelliği ve sakinliği savunan turizmdir. 'Cittaslow' (Sakin Şehir) unvanı alan yerlerde (Örn: Seferihisar) yapılır.",
          "examples": [
            "Örnekle Pekiştirelim: İzmir Seferihisar'a giden bir turistin, arabasını park edip sokaklarda yürüyerek yerel kadınların kooperatifte sattığı el açması börekleri yavaşça tatmasıdır."
          ]
        },
        {
          "name": "Y Kuşağı Turizmi",
          "desc": "1980-2000 arası doğan teknolojiye hâkim Y kuşağının (Millenials) klasik deniz tatili yerine, otantik, sırt çantalı ve dijital deneyimler aradığı turizm şeklidir.",
          "examples": [
            "Örnekle Pekiştirelim: Genç bir yazılımcının sırt çantasını alıp Likya Yolu'nda yürürken, akşam kaldığı çadırda cep telefonuyla sosyal medyada canlı yayın yapmasıdır."
          ]
        },
        {
          "name": "Uzay Turizmi",
          "desc": "Çok yüksek bütçeli, yörünge altı veya uzay istasyonlarına yapılan ticari ve turistik amaçlı sıradışı yolculuklardır.",
          "examples": [
            "Örnekle Pekiştirelim: Milyarder bir iş insanının özel bir uzay şirketiyle (SpaceX) anlaşarak, yer çekimsiz ortamı deneyimlemek üzere yörünge dışına uçmasıdır."
          ]
        }
      ]
    },
    {
      "id": 6,
      "tag": "DİĞER",
      "title": "KONGRE, GASTRONOMİ, YAT VE ETKİNLİK TURİZMİ",
      "microSummary": "İş, yeme-içme tutkusu veya özel hobiler için yapılan seyahat türleri.",
      "definitions": [
        {
          "name": "Gastronomi Turizmi",
          "desc": "Sadece bir yörenin özgün yeme-içme kültürünü tatmak (Gurme, Gastronom) amacıyla yapılan seyahatlerdir. Türk mutfağı dünyada büyük bir çekim merkezidir.",
          "examples": [
            "Örnekle Pekiştirelim: Gaziantep'e sadece sabah meşhur Beyran çorbası içmek, öğlen Ali Nazik kebabı yemek ve akşam fıstıklı baklava tatmak için düzenlenen lezzet turudur."
          ]
        },
        {
          "name": "Kongre Turizmi",
          "desc": "Bilimsel, mesleki veya ticari toplantılar için yapılan, büyük otellerde yılın her ayı gerçekleştirilebilen ve normal turistten çok daha fazla harcama yapan kitlenin katıldığı turizmdir.",
          "examples": [
            "Örnekle Pekiştirelim: İstanbul Lütfi Kırdar Kongre Merkezi'nde 3000 yabancı doktorun katılımıyla 4 gün süren uluslararası tıp kongresine katılmak için gelen heyetlerin seyahatidir."
          ]
        },
        {
          "name": "Yat ve Kruvaziyer Turizmi",
          "desc": "Yat kiralayarak koyları gezmek (Yat) veya yüzen 5 yıldızlı devasa otellerle uluslararası limanları ziyaret etmektir (Kruvaziyer).",
          "examples": [
            "Örnekle Pekiştirelim: Fethiye'den 12 kişilik bir ahşap gulet kiralayarak Göcek'in el değmemiş koylarında bir hafta boyunca 'Mavi Yolculuk' yapmaktır."
          ]
        },
        {
          "name": "Helal Turizm",
          "desc": "İslami kurallara uygun hizmet (helal gıda standartları, kadın/erkek ayrı havuzlar) veren tesislere yapılan seyahatlerdir.",
          "examples": [
            "Örnekle Pekiştirelim: Muhafazakar bir ailenin, helal gıda sertifikalı restoranları ve kadınlara özel korunaklı plajları olan Alanya'daki bir tatil köyünde konaklamasıdır."
          ]
        }
      ]
    }
  ]
};

const List<Map<String, dynamic>> alternatifTurizmNotes = [
  alternatifTurizmUnit1,
  alternatifTurizmUnit2,
];
